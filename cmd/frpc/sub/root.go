...
func init() {
	rootCmd.PersistentFlags().StringVarP(&cfgFile, "config", "c", "./client.ini", "config file of proxyc")
	rootCmd.PersistentFlags().StringVarP(&cfgDir, "config_dir", "", "", "config directory, run one proxyc service for each file in config directory")
	rootCmd.PersistentFlags().BoolVarP(&showVersion, "version", "v", false, "version of proxyc")
...
var rootCmd = &cobra.Command{
	Use:   "proxyc",
	Short: "proxyc is the client of proxy",
...
func startService(
	...
	if cfgFile != "" {
		defer log.Infof("proxy service for config file [%s] stopped", cfgFile)
	}
